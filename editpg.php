...
<?php 
require_once("managerSidebarpg.php");
...
$cnqu = "UPDATE `activity` SET `longitude` = '".$_GET['lon']."' ,`latitude` = '".$_GET['lat']."' WHERE `activity`.`id` = $id;";
	...
	header("location:editpg.php?id=$id");
...
if (isset($_POST['changecost'])) 
{
//$id= $_GET['id'];
$cnqu = "UPDATE `activity` SET `hrcost` = '".$_POST['hrcost']."' WHERE `activity`.`id` = $id;";
	echo $cnqu;
	mysqli_query($con, $cnqu);
	header("location:editpg.php?id=$id");
}

if (isset($_POST['changesport'])) 
{
//$id= $_GET['id'];
$cnqu = "UPDATE `activity` SET `sport` = '".$_POST['sport']."' WHERE `activity`.`id` = $id;";
	echo $cnqu;
	mysqli_query($con, $cnqu);
	header("location:editpg.php?id=$id");
}
...
<?php
 ...
 echo "<input type=number name='hrcost' value='$hr'>";

?>
<button type=submit name="changecost"> Change Hourly cost </button>
<br>
<?php
...
<?php
 ...
 echo "<input type=text name='sport' value='$sport'>";

?>
<button type=submit name="changesport"> Change Sport </button>
...
<?php
 ...
 echo "<textarea type=text name='notes' rows=9 cols=30> $notes </textarea>";
